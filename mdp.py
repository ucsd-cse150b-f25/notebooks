...
class GridMDP(MDP):
    ...
    def __repr__(self):
        """Return a string representation of the GridMDP showing the grid structure."""
        lines = []
        lines.append(f"GridMDP({self.cols}x{self.rows})")
        lines.append("=" * (self.cols * 8 + 1))

        # Display grid from top to bottom (reversed from internal representation)
        ...
        lines.append(f"Initial state: {self.init}")
        lines.append(f"Terminal states: {sorted(self.terminals)}")
        lines.append(f"Gamma (discount): {self.gamma}")
        lines.append(f"States: {len(self.states)}")
        lines.append("* indicates terminal state")
